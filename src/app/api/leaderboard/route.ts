...
interface CachedBoard { data: unknown[]; builtAt: string; expiresAt: number; }
declare global { var __GRID_LEADER_CACHE__: CachedBoard | undefined; } // eslint-disable-line no-var
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (global.__GRID_LEADER_CACHE__ && global.__GRID_LEADER_CACHE__.expiresAt > now) {
      return successResponse(
        { leaderboard: global.__GRID_LEADER_CACHE__.data, builtAt: global.__GRID_LEADER_CACHE__.builtAt, cached: true },
        200, request
      );
    }
    ...
    // Rank by lifetimeCreds (total earned + bonus) — NEVER by availableCreds
    ...
    global.__GRID_LEADER_CACHE__ = { data: leaderboard, builtAt, expiresAt: now + env.LEADERBOARD_CACHE_TTL_MS };
