...
export async function GET(request: NextRequest) {
  try {
    ...
    for (let i = 0; i < memberIds.length; i += BATCH) {
      await Promise.allSettled(
        memberIds.slice(i, i + BATCH).map(async (memberId) => {
          ...
          if (Math.abs(delta) > 0) {
            const mismatch = { memberId, storedAvailable: ledger.availableCreds, computedAvailable, delta };
    ...
    const response = NextResponse.json({
      success: true, checkedAt: new Date().toISOString(),
      totalChecked: memberIds.length, mismatchCount: mismatches.length,
      healthy: mismatches.length === 0, mismatches,
    });
