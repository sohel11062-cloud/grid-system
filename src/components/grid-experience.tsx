...
function CouponStatusBadge({ status }: { status: GridCouponRecord["status"] }) {
  const cls =
    ...
    status === "EXPIRED" ? "text-amber-300/60"    :
    status === "FAILED"  ? "text-red-400/80"      : "text-grid-muted";
  return (
    <p className={`mt-1 text-[10px] uppercase tracking-[0.28em] ${cls}`}>{status}</p>
  );
}

/** NEW — Transaction type pill */
function TxTypeBadge({ type }: { type: CreditTransaction["type"] }) {
  const cls =
    type === "EARN"       ? "text-grid-cyan/80"    :
    type === "BONUS"      ? "text-grid-violet/80"  :
    type === "REDEEM"     ? "text-grid-magenta/80" :
                            "text-grid-muted";
  const icon =
    type === "EARN"   ? "▲" :
    type === "BONUS"  ? "★" :
    type === "REDEEM" ? "▼" : "~";
  return (
    <span className={`inline-flex items-center gap-1 text-[10px] font-semibold uppercase tracking-[0.26em] ${cls}`}>
      {icon} {type}
    </span>
  );
}

/** NEW — Lifetime stats row */
function LifetimeStatsRow({ stats, lifetimeCreds }: { stats: LifetimeStats; lifetimeCreds: number }) {
  return (
    <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
      {[
        { label: "TOTAL EARNED",    value: formatCompactNumber(lifetimeCreds) + " C" },
        { label: "TOTAL SAVED",     value: formatIndianCurrency(stats.totalSavingsRupees) },
        { label: "COUPONS USED",    value: String(stats.totalCouponsUsed) },
        { label: "ACTIVE COUPONS",  value: String(stats.totalCouponsActive) },
      ].map((item) => (
        <div key={item.label} className="rounded-[18px] border border-white/8 bg-black/15 p-3 text-center">
          <p className="text-[9px] uppercase tracking-[0.36em] text-grid-muted">{item.label}</p>
        ...
        >
          <div className="flex items-center gap-3">
            <div
              className={[
                ...
                tx.type === "EARN"   ? "bg-grid-cyan/15 text-grid-cyan"    :
                tx.type === "BONUS"  ? "bg-grid-violet/15 text-grid-violet" :
                tx.type === "REDEEM" ? "bg-grid-magenta/15 text-grid-magenta" :
                                       "bg-white/5 text-grid-muted",
              ].join(" ")}
            >
            ...
            <div>
              ...
              <p className="mt-0.5 text-[10px] text-grid-muted truncate max-w-[140px]">
          ...
          <div className="text-right">
            <p className={[
              ...
              tx.type === "REDEEM" ? "text-grid-magenta" : "text-white",
            ].join(" ")}>
              {tx.type === "REDEEM" ? "-" : "+"}{tx.amount.toLocaleString("en-IN")} C
            </p>
            <p className="mt-0.5 text-[10px] text-grid-muted">{fmtDateShort(tx.createdAt)}</p>
          </div>
        </motion.div>
      ))}
    </div>
      ...
      <div className="mx-auto grid w-full max-w-6xl gap-8 xl:grid-cols-[1.2fr_0.8fr]">
        ...
        <motion.section {...fadeUp(0)} className="panel-shell flex flex-col justify-between p-8 md:p-10">
          <div>
            ...
            <h1 className="glitch-text mt-6 text-4xl uppercase leading-[0.9] tracking-[0.14em] text-white md:text-6xl" data-text="THE GRID">
              THE GRID
            </h1>
            ...
            <p className="mt-8 max-w-2xl text-base leading-7 text-grid-muted">
              Sign in with your Wix identity to unlock purchase-linked Creds, birthday boosts, dynamic
              tier elevation, and coupon redemptions — all tracked in a tamper-resistant audit ledger.
            </p>
          </div>
          <div className="mt-12 flex flex-wrap items-center gap-4">
            <motion.button whileHover={{ scale: 1.04, y: -2 }} whileTap={{ scale: 0.96 }} onClick={onLogin} className="grid-button min-w-52">
              Sign In To Enter
            </motion.button>
        ...
        <motion.aside {...fadeUp(0.12)} className="panel-shell p-8">
          ...
          <div className="mt-6 space-y-3 text-sm text-grid-muted">
            {[
              { lbl: "CRED PROTOCOL",  txt: "₹1 spent = 1 Cred with welcome and birthday rewards." },
              { lbl: "LIVE SYNC",      txt: "Member data refreshes on demand and via daily cron." },
              { lbl: "REDEEM ENGINE",  txt: "100 Creds = ₹1. Coupons created via Wix Stores API." },
              { lbl: "AUDIT LEDGER",   txt: "Every transaction is recorded for full traceability." },
            ].map((item) => (
              <motion.div key={item.lbl} whileHover={{ x: 4 }} transition={{ type: "spring", stiffness: 300, damping: 24 }} className="rounded-[22px] border border-white/10 bg-white/5 p-4">
  ...
  return (
    <main className="relative flex min-h-screen items-center justify-center px-6 py-16">
      ...
      <div className="panel-shell z-10 w-full max-w-md">
        ...
        <p className="mt-4 text-sm text-grid-muted">Calibrating loyalty ledger and holographic overlays...</p>
        <div className="progress-track mt-8">
          <div className="progress-fill animate-pulseLine w-full"><span className="progress-orb" /></div>
        </div>
      </div>
    </main>
  );
...
export function GridExperience() {
  ...
  const [showTx, setShowTx]             = useState(false);     // NEW: toggle tx history
  ...
  const txInView     = useInView(txRef,     { once: true, margin: "-60px" });
              ...
              <div className="relative z-10 mx-auto flex max-w-7xl flex-col gap-6">
                ...
                <motion.header {...fadeUp(0)} className="panel-shell flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <motion.button whileHover={{ scale:1.04 }} whileTap={{ scale:0.96 }} onClick={handleLogout} className="grid-button-ghost">Exit Session</motion.button>
                  </div>
                </motion.header>
                  ...
                  {latestCoupon && (
                    ...
                    >
                      ...
                      {latestCoupon.expiresAt && (
                        <span className="ml-2 text-grid-muted/70 text-xs">expires {fmtDateShort(latestCoupon.expiresAt)}</span>
                      )}
                    </motion.div>
                  )}
                </AnimatePresence>
                ...
                <section className="grid gap-6 xl:grid-cols-[1.35fr_0.95fr]">
                  ...
                  <motion.div {...fadeUp(0.08)}>
                    <TiltCard intensity={6}>
                      ...
                      {/* NEW: Lifetime stats row */}
                      <LifetimeStatsRow stats={dashboard.lifetimeStats} lifetimeCreds={dashboard.wallet.lifetimeCreds} />
                      ...
                      <div className="mt-6 rounded-[24px] border border-white/10 bg-black/20 p-5">
                        ...
                        <div className="progress-track mt-5">
                          <motion.div className="progress-fill" initial={{width:0}} animate={{width:`${Math.max(dashboard.wallet.progressRatio*100,2)}%`}} transition={{duration:1.2,ease:eSmooth}}>
                  ...
                  <motion.div {...fadeUp(0.14)} className="grid gap-6">
                    <TiltCard intensity={5}>
                      ...
                      <p className="mt-3 text-sm leading-7 text-grid-muted">
                        ...
                        Creds only deducted on confirmed creation.
                      </p>
                      <form className="mt-6 space-y-4" onSubmit={handleRedeem}>
                        <label className="block">
                          ...
                          <input className="grid-input" inputMode="numeric" min={100} step={100} value={credsInput} onChange={(e)=>setCredsInput(e.target.value)} placeholder="e.g. 1000" />
                        </label>
                ...
                {/* Bottom: Orders + Leaderboard/Coupons */}
                <section className="grid gap-6 lg:grid-cols-2">

                  <motion.div ref={ordersRef} initial={{opacity:0,y:40}} animate={ordersInView?{opacity:1,y:0}:{}} transition={{duration:0.7,ease:eSmooth}}>
                    <TiltCard intensity={4}>
                      ...
                      <div className="mt-6 space-y-3">
                        {dashboard.orders.length > 0 ? dashboard.orders.map((o, i) => (
                          <motion.div key={o.id} initial={{opacity:0,x:-14}} animate={ordersInView?{opacity:1,x:0}:{}} transition={{delay:0.06*i}} whileHover={{x:5}} className="rounded-2xl border border-white/10 bg-black/20 p-4">
                            <div className="flex flex-wrap items-center justify-between gap-3">
                              <div>
                                ...
                                {o.couponCode && (
                                  <p className="mt-1 text-[10px] text-grid-violet/80">
                                    Coupon: {o.couponCode}
                                  </p>
                                )}
                              </div>
                  ...
                  <motion.div ref={leaderRef} initial={{opacity:0,y:40}} animate={leaderInView?{opacity:1,y:0}:{}} transition={{duration:0.7,delay:0.1,ease:eSmooth}} className="grid gap-6">
                    ...
                    <TiltCard intensity={4}>
                      ...
                      <div className="mt-6 space-y-3">
                        {dashboard.leaderboard.length > 0 ? dashboard.leaderboard.map((e, i) => (
                          <motion.div key={e.memberId} initial={{opacity:0,x:14}} animate={leaderInView?{opacity:1,x:0}:{}} transition={{delay:0.06*i}} whileHover={{x:-5}} className="flex items-center justify-between rounded-2xl border border-white/10 bg-black/20 px-4 py-3">
                            <div className="flex items-center gap-4">
                              <div className={["flex h-10 w-10 shrink-0 items-center justify-center rounded-full border text-sm font-semibold text-white",
                                i===0?"border-grid-cyan/50 bg-grid-cyan/15":i===1?"border-grid-blue/40 bg-grid-blue/12":"border-white/10 bg-white/5"].join(" ")}>
                          ...
                          <div className="rounded-2xl border border-dashed border-white/10 bg-black/20 p-6 text-sm text-grid-muted">Leaderboard populates after first sync.</div>
                        )}
                      </div>
                    </TiltCard>

                    {/* Coupon ledger — enhanced with lifecycle status */}
                    <TiltCard intensity={4}>
                      <div className="flex items-center justify-between">
                        <div>
                      ...
                      {/* NEW: Coupon summary pills */}
                      <div className="mt-4 flex flex-wrap gap-2">
                        {[
                          { lbl: "Active",  val: dashboard.lifetimeStats.totalCouponsActive,  cls: "text-grid-cyan/80"   },
                          { lbl: "Used",    val: dashboard.lifetimeStats.totalCouponsUsed,    cls: "text-grid-violet/80" },
                          { lbl: "Expired", val: dashboard.lifetimeStats.totalCouponsExpired, cls: "text-amber-300/70"   },
                        ].map((p) => (
                          <span key={p.lbl} className={`inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[10px] uppercase tracking-[0.28em] ${p.cls}`}>
                            {p.val} {p.lbl}
                          </span>
                        ))}
                      </div>
                      <div className="mt-4 space-y-3">
                        {dashboard.coupons.length > 0 ? dashboard.coupons.map((c, i) => (
                          <motion.div key={c.id} initial={{opacity:0}} animate={leaderInView?{opacity:1}:{}} transition={{delay:0.07*i}} whileHover={{x:-5}} className="rounded-2xl border border-white/10 bg-black/20 p-4">
                            <div className="flex flex-wrap items-center justify-between gap-3">
                              <div>
                                <p className="font-mono text-sm font-medium text-white">{c.code}</p>
                                <CouponStatusBadge status={c.status} />
                                {c.usedAt && (
                                  <p className="mt-0.5 text-[10px] text-grid-muted">Used {fmtDateShort(c.usedAt)}</p>
                                )}
                              </div>
                              <div className="text-right">
                            ...
                            <div className="mt-2 flex items-center justify-between gap-2">
                              <p className="text-[10px] text-grid-muted">{fmtDate(c.createdAt)}</p>
                              {c.expiresAt && c.status === "ACTIVE" && (
                                <p className="text-[10px] text-amber-300/60">expires {fmtDateShort(c.expiresAt)}</p>
                              )}
                            </div>
                            {c.note && <p className="mt-1.5 text-[10px] text-amber-100/80">{c.note}</p>}
                          </motion.div>
                        )) : (
                          <div className="rounded-2xl border border-dashed border-white/10 bg-black/20 p-6 text-sm text-grid-muted">No coupons issued yet.</div>
                        )}
                      </div>
                    </TiltCard>
                  </motion.div>
                ...
                {/* NEW: Transaction History Panel */}
                <motion.div ref={txRef} initial={{opacity:0,y:40}} animate={txInView?{opacity:1,y:0}:{}} transition={{duration:0.7,ease:eSmooth}}>
                  <TiltCard intensity={3}>
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="panel-title">CREDIT LEDGER</p>
                        <h3 className="mt-3 text-xl uppercase tracking-[0.16em] text-white">Transaction History</h3>
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="data-chip">{dashboard.recentTransactions.length} recent</span>
                        <motion.button
                    ...
                    {/* Always show last 3 rows; full list when expanded */}
                    <AnimatePresence>
                      {(showTx || dashboard.recentTransactions.length <= 3) ? (
                        <motion.div key="full" initial={{opacity:0,height:0}} animate={{opacity:1,height:"auto"}} exit={{opacity:0,height:0}}>
                          <TransactionPanel transactions={dashboard.recentTransactions} inView={txInView} />
                        </motion.div>
                      ) : (
                        <motion.div key="preview" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
                          <TransactionPanel transactions={dashboard.recentTransactions.slice(0, 3)} inView={txInView} />
                          <p className="mt-3 text-center text-[11px] uppercase tracking-[0.3em] text-grid-muted/60 cursor-pointer hover:text-grid-muted transition-colors" onClick={() => setShowTx(true)}>
                            +{dashboard.recentTransactions.length - 3} more — click to expand
                          </p>
                        </motion.div>
                      )}
                    </AnimatePresence>
