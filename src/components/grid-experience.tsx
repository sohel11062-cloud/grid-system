...
function StatusDot({ status }: { status: "ONLINE" | "DEGRADED" | "SYNCING" }) {
  const cls = status === "ONLINE" ? "status-dot-online" : status === "SYNCING" ? "status-dot-syncing" : "status-dot-degraded";
  const lbl = status === "ONLINE" ? "text-emerald-400/80" : status === "SYNCING" ? "text-amber-300/80" : "text-red-400/80";
  return (
...
function TxBadge({ type }: { type: CreditTransaction["type"] }) {
  const icon = type === "EARN" ? "▲" : type === "BONUS" ? "★" : type === "REDEEM" ? "▼" : "~";
  const cls  = type === "EARN" ? "text-grid-cyan" : type === "BONUS" ? "text-grid-violet" : type === "REDEEM" ? "text-grid-magenta" : "text-grid-muted";
  return <span className={`text-[10px] font-semibold uppercase tracking-[0.26em] ${cls}`}>{icon} {type}</span>;
}
        ...
        <motion.section {...fadeUp(0)} className="panel-shell flex flex-col justify-between p-8 md:p-10">
          <div>
            ...
            <p className="mt-7 max-w-2xl text-sm leading-7 text-grid-muted">
              Sign in to unlock purchase-linked Creds, birthday boosts, dynamic tier elevation and reward
              coupon redemptions — all tracked in a tamper-resistant audit ledger.
            </p>
          </div>
          <div className="mt-10 flex flex-wrap items-center gap-3">
            <motion.button whileHover={{ scale: 1.04, y: -2 }} whileTap={{ scale: 0.96 }} onClick={onLogin} className="grid-button min-w-48">
              Sign In To Enter
            </motion.button>
...
export function GridExperience() {
  ...
  async function load() {
    try {
      ...
      if ((e as Error & { status?: number }).status === 401) { setAuth(true); setDashboard(null); }
      else setError((e as Error).message ?? "Could not load dashboard.");
  ...
  async function handleSync() {
    ...
    } catch (e) { setError((e as Error).message ?? "System sync failed. Please try again."); }
  ...
  async function handleRedeem(ev: React.FormEvent) {
    ...
    try {
      ...
      const p = await gFetch<{ coupon: GridCouponRecord; dashboard: GridDashboardData }>("/api/redeem", {
        method: "POST",
        body:   JSON.stringify({ creds: Number(credsInput) }),
      });
      setDashboard(p.dashboard);
      setLatest(p.coupon);
    } catch (e) { setError((e as Error).message ?? "Reward engine failed. Please try again."); }
      ...
      <div className="relative z-10 mx-auto flex max-w-7xl flex-col gap-6">
        ...
        <motion.header {...fadeUp(0)} className="panel-shell flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <h1 className="glitch-text mt-2 text-3xl uppercase tracking-[0.18em] text-white md:text-5xl" data-text="THE GRID">THE GRID</h1>
        ...
        <AnimatePresence>
          ...
          {latestCoupon && (
            <motion.div key="cpn" initial={{ opacity:0, y:-8 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0 }}
              ...
              {latestCoupon.expiresAt && <span className="ml-2 text-[11px] text-grid-muted/60">expires {fmtShort(latestCoupon.expiresAt)}</span>}
            </motion.div>
          )}
        </AnimatePresence>
          ...
          <motion.div {...fadeUp(0.13)} className="grid gap-6">
            <TiltCard intensity={4}>
              ...
              <p className="mt-2 text-xs leading-6 text-grid-muted">
                100 Creds = ₹1 · Reward coupons issued via Processing Engine · Creds deducted on confirmation only
              </p>
              <form className="mt-5 space-y-4" onSubmit={handleRedeem}>
                <label className="block">
                  ...
                  <input className="grid-input" inputMode="numeric" min={100} step={100}
                    value={credsInput} onChange={(e) => setCredsInput(e.target.value)} placeholder="e.g. 1000" />
                </label>
            ...
            <TiltCard intensity={3}>
              <div className="flex items-center justify-between">
                <div>
                  <p className="panel-title">SYSTEM STATUS</p>
                  <h3 className="mt-2 text-base uppercase tracking-[0.16em] text-white">Ledger: {d.system.syncWindowLabel}</h3>
                </div>
        ...
        <section className="grid gap-6 lg:grid-cols-2">
          ...
          {/* Orders */}
          <motion.div ref={ordersRef} initial={{ opacity:0, y:36 }} animate={ordersIn ? { opacity:1, y:0 } : {}} transition={{ duration:0.65, ease }}>
            <TiltCard intensity={3}>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <motion.div key={o.id}
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div>
                        ...
                        <p className="mt-0.5 text-[10px] uppercase tracking-[0.22em] text-grid-muted">{o.status} · {o.paymentStatus}</p>
                        {o.couponCode && <p className="mt-0.5 text-[10px] text-grid-violet/80">Coupon: {o.couponCode}</p>}
                      </div>
          ...
          {/* Coupons + Leaderboard */}
          <motion.div ref={cpnRef} initial={{ opacity:0, y:36 }} animate={cpnIn ? { opacity:1, y:0 } : {}} transition={{ duration:0.65, delay:0.08, ease }}
            ...
            <TiltCard intensity={3}>
              ...
              <div className="mt-4 space-y-3">
                {d.coupons.length > 0 ? d.coupons.slice(0, 4).map((c, i) => (
                  <motion.div key={c.id}
                    initial={{ opacity:0 }} animate={cpnIn ? { opacity:1 } : {}} transition={{ delay:0.06*i }}
                    className="rounded-2xl border border-white/10 bg-black/20 p-4">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div>
        ...
        {/* ── Transaction History ──────────────────────────────────────────── */}
        <motion.div ref={txRef} initial={{ opacity:0, y:36 }} animate={txIn ? { opacity:1, y:0 } : {}} transition={{ duration:0.65, ease }}>
          <TiltCard intensity={2}>
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2.5">
                ...
                <motion.button whileHover={{ scale:1.03 }} whileTap={{ scale:0.97 }}
                  onClick={() => setShowTx((p) => !p)} className="grid-button-ghost text-[10px]">
                ...
                <div className="space-y-2.5">
                  {(showTx ? d.recentTransactions : d.recentTransactions.slice(0, 3)).map((tx, i) => (
                    <motion.div key={tx.id}
                      initial={{ opacity:0, x:10 }} animate={txIn ? { opacity:1, x:0 } : {}} transition={{ delay:0.04*i }}
                  ...
                  {!showTx && d.recentTransactions.length > 3 && (
                    <p className="cursor-pointer text-center text-[10px] uppercase tracking-[0.3em] text-grid-muted/50 transition-colors hover:text-grid-muted"
